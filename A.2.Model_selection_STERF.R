...
Selection_sterf <- data.frame(matrix(nrow=0, ncol=length(columns)))
colnames(Selection_sterf) <- columns

for (e in unique(glmm.TMB.nb$GROUP)){
  if (glmm.TMB$AIC[glmm.TMB$GROUP==e] <= glmm.TMB.nb$AIC[glmm.TMB.nb$GROUP==e]) {Selection_sterf <- rbind(Selection_sterf, data.frame(GROUP=e, 
                                                                                                                                  Modele="Poisson", 
  ...
  else {Selection_sterf <- rbind(Selection_sterf, data.frame(GROUP=e, 
                                                         Modele="NB", 
