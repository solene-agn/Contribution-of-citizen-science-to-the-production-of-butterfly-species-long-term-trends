...
# Poisson
glmm.TMB <- read.csv2("data/UKBMS/TMB_TT/TMB/ukBMS_month.TMB.csv", sep=";")
...
glmm.TMB <- glmm.TMB %>% mutate_if(is.numeric,~round(.,3))
glmm.TMB <- glmm.TMB[(glmm.TMB$Effect=="YEAR_sc"),]
names(glmm.TMB)[names(glmm.TMB)=="Esp"] <- "GROUP"
glmm.TMB[is.na(glmm.TMB)] <- "NA"
glmm.TMB$AIC <- as.numeric(as.character(glmm.TMB$AIC))

# Negative binomial
glmm.TMB.nb <- read.csv2("data/UKBMS//TMB_TT/TMB_nb/ukBMS_month.TMB.NB.csv", sep=";")
...
glmm.TMB.nb <- glmm.TMB.nb %>% mutate_if(is.numeric,~round(.,3))
glmm.TMB.nb <- glmm.TMB.nb[(glmm.TMB.nb$Effect=="YEAR_sc"),]
...
Selection_ukbms <- data.frame(matrix(nrow=0, ncol=length(columns)))
colnames(Selection_ukbms) <- columns

for (e in unique(glmm.TMB.nb$GROUP)){
  if (glmm.TMB$AIC[glmm.TMB$GROUP==e] <= glmm.TMB.nb$AIC[glmm.TMB.nb$GROUP==e]) {Selection_ukbms <- rbind(Selection_ukbms, data.frame(GROUP=e, 
                                                                                                                                  Modele="Poisson", 
  ...
  else {Selection_ukbms <- rbind(Selection_ukbms, data.frame(GROUP=e, 
                                                         Modele="NB", 
...
Selection_ukbms <- dplyr::distinct(Selection_ukbms, GROUP, .keep_all=TRUE)
...
Selection_ukbms <- rbind(Selection_ukbms, missing)

write.table(Selection_ukbms, "data/ukbms/TMB_TT/Trend_ukbms.csv", sep=";", row.names=FALSE)
